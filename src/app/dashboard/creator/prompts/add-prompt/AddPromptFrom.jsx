...
export default function AddPromptForm({ user }) {
  ...
  const hasReachedCreatorLimit =
    creatorPromptCount !== null &&
    !isPremium &&
    creatorPromptCount >= creatorLimit;
    ...
    const fetchCreatorPromptCount = async () => {
      ...
      try {
        const res = await fetch(
          `${API_URL}/api/prompts?creatorEmail=${user.email}&page=1&perPage=1`,
          { cache: "no-store" }
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        creatorId: user._id || user.id || user.email,
        ...
        status: "pending",
        isVisible: false,
        requestedVisibility: visibility.toLowerCase(),
        featured: false,
      };
      ...
      <section className="mx-auto max-w-5xl">
        <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] p-6 shadow-2xl shadow-black/30 backdrop-blur">
          ...
          <div className="mb-8 flex flex-col gap-3 border-b border-white/10 pb-6">
            ...
            {isPremium && (
              <div className="w-fit rounded-full border border-yellow-400/30 bg-yellow-400/10 px-4 py-2 text-sm font-bold text-yellow-200">
                Premium account active: unlimited prompt submissions
              </div>
            )}
          </div>
            ...
            <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
              ...
              <div>
                  ...
                  {["Public", "Private"].map((option) => (
                    ...
                    >
                      ...
                      <p className="mt-1 text-sm text-white/45">
                        {option === "Public"
                          ? "Will become public after admin approval."
                          : "Premium users can unlock after admin approval."}
                      </p>
                    </button>
                  ))}
                </div>
                ...
                <div className="mt-6 rounded-2xl border border-white/10 bg-[#121322]/70 p-5">
                  ...
                  <p className="mt-2 text-xs font-bold uppercase tracking-wider text-[#967bb6]">
            ...
            {hasReachedCreatorLimit ? (
              <div className="space-y-4">
                <div className="mb-4 rounded-3xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-200">
                  Free users can add only <strong>3 prompts</strong>. Upgrade
                  to Premium to add more.
                </div>
                ...
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() =>
                      router.push("/payment?redirect=/dashboard/creator/prompts")
                    }
                    className="h-12 flex-1 rounded-xl bg-gradient-to-r from-[#967bb6] to-[#8a96ce] text-base font-bold text-white shadow-md"
                  >
                    Upgrade to Premium
                  </button>
                  ...
                  <button
                    ...
                    className="h-12 flex-1 rounded-xl border border-white/10 bg-transparent text-base font-bold text-white/70"
                  >
                    Manage Prompts
                  </button>
                </div>
