...
export default function AdminPromptsTable({ prompts = [] }) {
  ...
  const confirmReject = async () => {
    ...
    setLoading(rejectModal._id);

    try {
      const result = await updatePromptStatus(rejectModal._id, {
        status: "rejected",
        rejectionFeedback,
      });
  ...
  const confirmDelete = async () => {
    ...
    try {
      const result = await deletePromptByAdmin(deleteModal._id);
        ...
        {prompts.length === 0 ? (
          ...
          prompts.map((prompt) => {
            ...
            return (
              <article
                key={prompt._id}
                className="grid gap-5 rounded-3xl border border-white/10 bg-white/[0.04] p-5 shadow-xl shadow-black/20 transition hover:border-[#967bb6]/40 lg:grid-cols-[180px_1fr_auto]"
              >
                ...
                <div className="flex flex-row flex-wrap gap-2 lg:w-36 lg:flex-col">
                  ...
                  {status !== "rejected" ? (
                    <Action
                      disabled={isLoading}
                      onClick={() => openReject(prompt)}
                      icon={X}
                      label="Reject"
                      tone="warning"
                    />
                  ) : (
                    <Action disabled icon={X} label="Rejected" tone="warning" />
                  )}
      ...
      {rejectModal && (
        <Modal onClose={() => setRejectModal(null)}>
          ...
          <p className="mt-2 text-sm leading-6 text-white/45">
            Add clear feedback so the creator understands why this prompt did
            not meet the platform guidelines.
          </p>
          ...
          <div className="mt-5 flex justify-end gap-3">
            ...
            <button
              ...
              disabled={loading === rejectModal._id}
      ...
      {deleteModal && (
        <Modal onClose={() => setDeleteModal(null)}>
          ...
          <p className="mt-3 text-sm leading-6 text-white/45">
            This will permanently remove the prompt from the database and clear
            related reports. This action cannot be undone.
          </p>
          ...
          <div className="mt-5 flex justify-end gap-3">
            ...
            <button
              ...
              disabled={loading === deleteModal._id}
